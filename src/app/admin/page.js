/**
 * @file page.js
 * @description Admin panel for managing wedding guest list and dashboard. Uses Firebase Authentication (email + password) for login.
...
export default function Admin() {
  ...
  const [email, setEmail] = useState(""); // For login form
  ...
  useEffect(() => {
    ...
    if (user) fetchGuests(); // Only fetch guests if logged in
  }, [user]);
  ...
  return (
    <div className="relative font-sans min-h-screen flex flex-col items-center max-sm:pb-[80px] bg-gray-100 py-4 sm:px-4">
      ...
      {user ? (
        ...
        <div className="flex flex-col items-center">
          ...
          <button
            onClick={handleLogin}
            className="bg-blue-500 hover:bg-blue-600 text-white py-2 px-4 rounded mb-4 font-semibold"
          >
            Login
          </button>
          <button
